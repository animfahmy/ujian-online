...
class Slug
{
	...
	private function _check_uri($uri, $id = FALSE, $count = 0)
	{
		...
		if ($this->database != 'default'){
			$portal = $CI->load->database($this->database, true);
			// Setup the query
			$portal->select($this->field)->where($this->field, $new_uri);

			if ($id)
			{
				$portal->where($this->id.' !=', $id);
			}

			if ($portal->count_all_results($this->table) > 0)
			{
